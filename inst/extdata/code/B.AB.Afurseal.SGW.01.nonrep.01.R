...
# Function:            B.AB.Afurseal.SGW.01.nonrep.01
# Description:        Biological Element, Group Air-breathers, Antarctic Fur Seal - South Georgia - non-reproductive
...
#         of this element
    ...
    Taxon$Signature <- list(
      ID           = 25003,
      Name.full    = "Ant.Fur.Seal nonreprod - South Georgia West",
      Name.short   = "Afurseal NR SGW",
      Morph        = "01",
      Version      = "01",
      Authors      = "A.Constable",
...
#   the Universe e.g. natural mortality rate
    ...
    NatMort <- 0.05 # natural mortality rate
...
#   list of life stages to be monitored in State
    ...
    Life.stages<-list(Stage.names=c("NR"),Stage.ages = c(7))
...
#   change of juveniles to non-reproductives
          ...
          AntFurSealReprod <- 25004 # ID number of element to which pups go to juveniles
...
# pups from this element go into 4th element of juvenile relating to grid cells 19 & 24
    ...
    Change.element.data.01<-list(
                  new.element = AntFurSealReprod,
                  prop.to.change = list(
               AP     = c(0.5),      # Antarctic Peninsula
               EI     = c(0.5),     # Elephant Island
               SOI    = c(0.5),    # South Orkney Islands
               SGI    = c(0.5),    # South Georgia Island
               NthWed = c(0.5),    # North Weddell Sea
               SthWed = c(0.5),    # South Weddell Sea
               CenWed = c(0.5),       # Central Weddell Sea
               EAP    = c(0.5)        # East Antarctic Peninsula
                            ), # end prop.to.change
        new.element.stage.classes=list(
               AP     = list(        # Antarctic Peninsula
                          S1 = matrix(c(1,1,1),nrow=1,ncol=3)),
...
#   #############################################################
#   Individual reproductive condition in each polygon
...
#                       specify which dataset to use in each polygon
                        UseRdata        = c(1,1,1,1,1,1,1,1)   #     size at age
                    ) # # end Cond.S.data.01
...
#   Estimate abundances for each polygon at 1 January
    ...
    # abundance of nonreproductives
    NonReprod<-10000*exp(-2.303*0.4-0.6*0.357)/(exp(0.163)-1)

    # half are breeding at 1 Jan

    NonReprod<-NonReprod/2


    Init.ab<-NonReprod*AbN/Taxon$Polygons$Polygon.N #abundance per polygon - distribute evenly to seed the trial
    SumInitAb<-sum(Init.ab)
...
#   knife-edge functions can be included by repeating the same day
    ...
    Taxon$TimeSteps<-list(
       ...
       Summer_1     = list(calday=U.ST.DayFromDate.01(7,1),actions.N=2,actions=list(
...
#       note comma is used to append this to list
       ...
       , Adopt.reprod     = list(calday=U.ST.DayFromDate.01(1,4),actions.N=3,actions=list(
                      consume     = list(Update.fn = B.FO.Predation.02.01,
                                         dSet      = Consume.data.01),
                      migrate     = list(Update.fn = B.PD.migrate.01,    # function to be used
                                         dSet      = Migration.data.01), # pattern for input to function
                      update.rep.cond = list(Update.fn = B.PD.update_reprod.cond.01,    # function to be used
                                         dSet      = Cond.R.data.01), # pattern for input to function
                      adopt       = list(Update.fn = B.AB.Afurseal.adopt.01,# function to be used
                                         dSet      = list())       # pattern for input to function
          ) # end actions list
        ) # end time step adopt reproductive adults

       , Adopt.juve     = list(calday=U.ST.DayFromDate.01(26,9),actions.N=3,actions=list(
                      consume     = list(Update.fn = B.FO.Predation.02.01,
                                         dSet      = Consume.data.01),
                      migrate     = list(Update.fn = B.PD.migrate.01,    # function to be used
                                         dSet      = Migration.data.01), # pattern for input to function
        ...
        ) # end time step adopt juveniles

       , Depart     = list(calday=U.ST.DayFromDate.01(30,9),actions.N=3,actions=list(
                      consume     = list(Update.fn = B.FO.Predation.02.01,
                                         dSet      = Consume.data.01),
       ...
       , Spring     = list(calday=U.ST.DayFromDate.01(1,12),actions.N=2,actions=list(
...
#   function to undertake setup actions such as to change ID linkages to references in the relevant components
    ...
    Taxon$Ancillary.fns<-list(
        setup         = B.AB.Afurseal.SGW.01.nonrep.setup.01,
