...
# Function:           C.01.epoc.05.01
# Description:        Ecosystem Productivity Ocean and Climate
...
#   Specify the universe for the epoc scenario
    ...
    Universe<-E.universe.05.01()

#   set trial state variables in an environment

    Trial.State<-E.trial.05.01()

#   ################################################################################
#   establish an annual diary of periods in which actions take place
#   ################################################################################

    Diary<-C.DI.Diary.01.01(Universe)
...
#   ################################################################################
    ...
    C.SE.Setup.universe.01(Universe,Diary)
...
#   until the end of the period when the State of each component is updated
    ...
    C.SE.Transition.01(Universe, Diary)

#   ################################################################################
#   output diagnostic files from setup

    C.DI.Diary.01.output.01(Universe,Diary)
...
print(paste("Year: ",as.character(yr),",  Period: ",as.character(pe)),sep="")
        ...
        C.PJ.Period.01(Universe,pe,Diary[[pe]],Trial.State)
if(pe==1) {
 KrillAb    <-rbind(KrillAb,    c(yr,((yr+Diary[[pe]]$Day/365)-1),sum(Universe$Biota[[1]]$State$Abundance[[MonUnits]])))
 AFSpupab   <-rbind(AFSpupab  , c(yr,((yr+Diary[[pe]]$Day/365)-1),sum(Universe$Biota[[3]]$State$Abundance[[MonUnits]])))
 AFSjuveab  <-rbind(AFSjuveab  ,c(yr,((yr+Diary[[pe]]$Day/365)-1),sum(Universe$Biota[[4]]$State$Abundance[[MonUnits]])))
 AFSnonrepab<-rbind(AFSnonrepab,c(yr,((yr+Diary[[pe]]$Day/365)-1),sum(Universe$Biota[[5]]$State$Abundance[[MonUnits]])))
 AFSrepab   <-rbind(AFSrepab  , c(yr,((yr+Diary[[pe]]$Day/365)-1),sum(Universe$Biota[[6]]$State$Abundance[[MonUnits]])))
 KrillArea<-rbind(KrillArea,c(((yr+Diary[[pe]]$Day/365)-1),Universe$Biota[[1]]$State$Abundance[[MonUnits]]))
