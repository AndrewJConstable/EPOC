...
# pups from this element go into 4th element of juvenile relating to grid cells 19 & 24
    ...
    Change.element.data.01<-list(
                  new.element = AntFurSealjuvenile,
                  prop.to.change = list(c(1)),
                  new.element.stage.classes=list(list(matrix(c(4,1,1),nrow=1,ncol=3)))
                  ) # end list
...
# health at age is at 1 January
        Taxon.health.at.age.Default <- c(1)
...
# declare State
    ...
    Taxon$State<-list(
       ...
       Cond.H   = list(
               SGW    = Taxon.health.at.age.Default
               ) # end Cond.H list
      ) # end list of State
