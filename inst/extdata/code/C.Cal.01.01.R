...
#      Signature <- list(
#        ID           =  11004,
#        Name.full    = "Ecosystem Calendar 01",
#        Name.short   = "Cal01",
#        Version      = "01",
#        Authors      = "A.Constable",
...
# In order not to update every component at every event in the calendar,
...
# create a list of each period in the calendar (between unique timesteps) that indicates the ecosystem components and
# the timestep for that component that is occurring in the calendar period and the proportion of the timestep that will
...
# initialise calendar
      ...
      for (eTSD in 1:PeriodN) {
        ...
        Calendar<-c(Calendar,list(Period))
      } # end etSD

# fill Calendar with relevant data

      Calendar<-c(Calendar,list(Period.N = PeriodN))
...
#           save the characteristics of the time step for that taxon
            Calendar[[eTSD]]$Biota[bn,1]<-TS.current                      # the number of the timestep for that element in the period
            Calendar[[eTSD]]$Biota[bn,2]<-Prop.year/TS.current.prop.year  # the proportion of the time step within the period
            Calendar[[eTSD]]$Biota[bn,3]<-Period.step                     # the number ofthe period in sequence within the timestep of that element
...
#           - add the last value used in "Period.step" to the values in the rest of the timestep in the early part of the year
            ...
            for (eTSD in 1:(eTSD.start.first.timestep-1)) {
              if (Calendar[[eTSD]]$Biota[bn,1]==1) Calendar[[eTSD]]$Biota[bn,3]<-Calendar[[eTSD]]$Biota[bn,3]+Period.step
            }
        } # end bn
} # end  StepBioata
